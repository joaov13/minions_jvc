...
class New_game:
    first_screen_on = True
    entire_game_on = True
    choose_game_mode_on = True
...
def process_screen_game(current_game):
    ...
    while current_game.screen_game_on:
        ...
        '''SPAWN OBJECTS'''
        obj_yellow_minion = current_game.screen.blit(image_yellow_minion, (
            objects_cords.cord_x_yellow_minion, objects_cords.cord_y_yellow_minion))
        ...
        if current_game.level == 'easy':
            obj_unicorn = current_game.screen.blit(image_unicorn,
                                                   (objects_cords.cord_x_unicorn, objects_cords.cord_y_unicorn))
            obj_banana = current_game.screen.blit(image_banana,
                                                  (objects_cords.cord_x_banana, objects_cords.cord_y_banana))
            obj_purple_minion = current_game.screen.blit(image_purple_minion, (
                objects_cords.cord_x_purple_minion, objects_cords.cord_y_purple_minion))
            ...
            obj_green_minion = current_game.screen.blit(image_green_minion, (
                objects_cords.cord_x_green_minion, objects_cords.cord_y_green_minion))
            obj_red_minion = current_game.screen.blit(image_red_minion, (
                objects_cords.cord_x_red_minion, objects_cords.cord_y_red_minion))
            obj_purple_minion = current_game.screen.blit(image_purple_minion, (
                objects_cords.cord_x_purple_minion, objects_cords.cord_y_purple_minion))
        ...
        if test_collision_red:
            ...
            if obj_symbol.colliderect(obj_banana):
                ...
                current_game.sound_of_collision.play()
                pygame.time.delay(50)
            if obj_symbol.colliderect(obj_purple_minion) or obj_symbol.colliderect(
                    obj_green_minion) or obj_symbol.colliderect(obj_red_minion):
                current_game.collision_counter += 1
                current_game.score_counter -= 10
...
def process_first_screen(current_game):
    while current_game.first_screen_on:
        current_game.screen.blit(image_first_screen, (0, 0))
        mouse = pygame.mouse.get_pos()
        print(mouse)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                current_game.first_screen_on = current_game.entire_game_on = False
        '''PLAY'''
        if is_hovering_play_first_screen(mouse):
            current_game.screen.blit(image_button_play_white, (389, 259))
            if pygame.mouse.get_pressed() == (1, 0, 0):
                current_game.sound_of_clicking.play()
                current_game.first_screen_on = False
                current_game.screen_game_on = True
                pygame.time.delay(200)
...
def process_choose_game_mode(info_game):
    while info_game.choose_game_mode_on:
        info_game.screen.blit(image_choose_game_mode_screen, (0, 0))
        mouse = pygame.mouse.get_pos()
        print(mouse)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                info_game.choose_game_mode_on = info_game.entire_game_on = False
        '''PLAY'''
        if is_hovering_play_down(mouse):
            info_game.screen.blit(image_button_play_white, (391, 584))
            if pygame.mouse.get_pressed() == (1, 0, 0):
        ...
        if is_hovering_classic(mouse):
            info_game.screen.blit(image_button_classic_white, (137, 270))
        ...
        if is_hovering_dodge(mouse):
            info_game.screen.blit(image_button_dodge_white, (137, 370))
            info_game.screen.blit(image_dodge_info, (581, 270))
            if pygame.mouse.get_pressed() == (1, 0, 0):
                info_game.sound_of_clicking.play()
                pygame.time.delay(200)
        '''BIG'''
        if is_hovering_big(mouse):
            info_game.screen.blit(image_button_big_white, (137, 470))
            info_game.screen.blit(image_big_info, (581, 270))
            if pygame.mouse.get_pressed() == (1, 0, 0):
                info_game.sound_of_clicking.play()
                pygame.time.delay(200)
...
while info_game.entire_game_on:
    clock.tick(FPS)
